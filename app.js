...
function buildSidebar(filter = "") {
  ...
  TOPICS.forEach(topic => {
    ...
    matchingSubs.forEach(sub => {
      ...
      item.className = "nav-item" + (currentSubtopic && currentSubtopic.id === sub.id ? " active" : "");
...
function renderCurrentTab() {
  if (activeTab === "gdc") {
    // GDC tab doesn't need a selected subtopic
    if (typeof renderGDCGuide === "function") renderGDCGuide();
    return;
  }
  if (!currentSubtopic) return;
...
function renderNotes() {
  ...
  // filter bar
  let filterHTML = `<div class="notes-filter-bar">
    <button class="filter-btn ${activeFilter.level === 'all' ? 'active' : ''}" onclick="setFilter('level','all')">All</button>
    <button class="filter-btn filter-sl ${activeFilter.level === 'SL' ? 'active' : ''}" onclick="setFilter('level','SL')">SL only</button>
  ...
  filtered.forEach((note, idx) => {
    cardsHTML += buildNoteCard(note, idx);
  });
  ...
  container.innerHTML = filterHTML + cardsHTML + addBtn;
  renderMath(container);
}

function buildNoteCard(note, idx) {
  const typeLabels = { formula: "Formula", tip: "Tip", mistake: "Common mistake", diagram: "Diagram" };
...
function addNote() {
  ...
  renderNotes();
  // immediately open edit on the new card
...
function escapeForDisplay(str) {
  if (!str) return "";
  // If content starts with an HTML tag, pass through directly (raw HTML tables etc.)
  if (/^\s*<(table|div|span|p|ul|ol|li|h[1-6]|pre|code)/i.test(str)) return str;
  // Split on math blocks, escape plain text, preserve LaTeX unchanged
  const mathRegex = /(\$\$[\s\S]+?\$\$|\$[^$\n]+?\$)/g;
  const parts = [];
...
function setupInlinSearch() {
  ...
  input.addEventListener("focus", () => {
    // if user starts typing in sidebar search, just filter
  });
}
...
function doSearch(q) {
  ...
  const seen = new Set();
  const unique = results.filter(r => { if (seen.has(r.id + r.snippet)) return false; seen.add(r.id + r.snippet); return true; });

  container.innerHTML = unique.slice(0, 12).map((r, i) => `
    <div class="search-result" data-id="${r.id}" onclick="selectSearchResult('${r.id}')">
